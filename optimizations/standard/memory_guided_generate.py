...
def generate(
    ...
    memory_available = memory_interface is not None and hasattr(memory_interface, 'initialized') and memory_interface.initialized
    ...
    while start_idx < total_len:
        ...
        for step in range(steps):
            # Calculate progress
            total_progress = (current_block * steps + step) / (((total_len - input_len - 1) // block_length + 1) * steps)
                            ...
                            try:
                                ...
                                if vector_db is not None:
                                    ...
                                    if similar_indices:
                                        ...
                                        logger.info(f"Found similar vector: {meta.get('type', 'unknown')} - {meta.get('text', '')}")
                                            ...
                                            for idx in range(len(vector_db.vectors)):
                                                ...
                                                if gen_meta.get('type') == 'generation' and \
                                                   gen_meta.get('prompt', '') == meta.get('text', ''):
                                                    vector_guidance = vec
                                                    logger.info(f"Using guidance from previous generation: {gen_meta.get('text', '')}")
                                                    break
