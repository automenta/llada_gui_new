...
from core.config import WINDOW_TITLE, WINDOW_WIDTH, WINDOW_HEIGHT, SPLITTER_RATIO, DEFAULT_PARAMS
...
try:
    from core.memory_management.integration import create_memory_status_widget, get_memory_manager, suggest_memory_optimizations
...
class LLaDAGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        if torch.cuda.is_available():
            try:
                ...
                if gpu_memory < 8:
                    ...
                    self.status_label.setText(f"Limited GPU memory ({gpu_memory:.1f}GB). Memory-saving options enabled.")
        ...
        # Add resize button
        self.resize_btn = QPushButton("Maximize")
        self.resize_btn.setToolTip("Toggle window maximize/restore (F11)")
        self.resize_btn.clicked.connect(self.toggle_maximize)
        button_layout.addWidget(self.resize_btn)
        
        # Add keyboard shortcut for maximize/restore
            ...
            try:
                ...
                if not memory_manager.can_fit_in_memory(config):
                    ...
                    if changes:
                        ...
                        msg.setInformativeText("Would you like to automatically adjust parameters to fit available memory?")
                        msg.setDetailedText("• " + "\n• ".join(changes))
    ...
    def get_generation_config(self):
        ...
        # If fast mode is enabled, adjust parameters for better performance
        gen_length = self.gen_length_spin.value()
        steps = self.steps_spin.value()
        block_length = self.block_length_spin.value()
        
        # For fast mode, reduce steps by 40% if the value is high enough
        if fast_mode and steps > 32:
            # Reduce steps but keep it a multiple of block divisions
            blocks = gen_length // block_length
            steps_per_block = max(8, (steps // blocks) * 6 // 10)  # 60% of original, minimum 8
            steps = steps_per_block * blocks
        ...
        return {
            ...
            'use_memory': self.memory_integration.isChecked(),
            'fast_mode': fast_mode
        }
    ...
    def update_memory_info(self, stats):
        ...
        self.system_memory_progress.setValue(int(system_percent))
        
        # Set system progress bar color based on usage
        if system_percent > 90:
            self.system_memory_progress.setStyleSheet("QProgressBar::chunk { background-color: red; }")
        elif system_percent > 75:
            self.system_memory_progress.setStyleSheet("QProgressBar::chunk { background-color: orange; }")
        else:
            self.system_memory_progress.setStyleSheet("")
        
        # Update GPU progress bar if available
        if stats.get('gpu_available', False):
            gpu_percent = stats.get('gpu_percent', 0)
            self.gpu_memory_progress.setValue(int(gpu_percent))
            
            # Set GPU progress bar color based on usage
            if gpu_percent > 90:
                self.gpu_memory_progress.setStyleSheet("QProgressBar::chunk { background-color: red; }")
                self.gpu_memory_label.setStyleSheet("color: red; font-weight: bold")
            elif gpu_percent > 75:
                self.gpu_memory_progress.setStyleSheet("QProgressBar::chunk { background-color: orange; }")
    ...
    def initialize_memory_if_enabled(self):
        ...
        if memory_enabled:
            try:
                from core.memory.memory_integration import initialize_memory, initialize_server_manager, get_server_manager
...
<li>Memory integration for context-aware generation</li>
<li>CPU fallback for low-memory situations</li>
<li>Extreme Memory Mode for running on GPUs with limited VRAM (8-12GB)</li>
</ul>

<h3>Memory Integration Features:</h3>
<ul>
<li>Long-term memory across generations</li>
<li>Improved coherence and context-awareness</li>
<li>Memory state visualization</li>
<li>Trainable on your generations</li>
<li>Save and load memory states</li>
</ul>
...
4. Toggle memory integration if you want context-aware generation<br>
5. Click "Generate" to start the diffusion process<br>
6. View the generated output in the "Text Output" tab<br>
7. Switch to the "Diffusion Visualization" tab to see the generation process</p>
...
"""
    ...
    def on_gen_length_changed(self, new_value):
        ...
        if new_value % block_length != 0:
            ...
            # Show info message
            self.status_label.setText(f"Generation length adjusted to {adjusted_value} to be compatible with block length {block_length}")
            QTimer.singleShot(3000, lambda: self.status_label.setText(""))
    ...
    def on_block_length_changed(self, new_value):
        ...
        if gen_length % new_value != 0:
            ...
            # Show info message
            self.status_label.setText(f"Generation length adjusted to {adjusted_gen_length} to be compatible with block length {new_value}")
            QTimer.singleShot(3000, lambda: self.status_label.setText(""))
    ...
    def toggle_maximize(self):
        ...
        if self.isMaximized():
            self.showNormal()
            self.resize_btn.setText("Maximize")
        else:
            self.showMaximized()
            self.resize_btn.setText("Restore")
